...
try:
    ...
    # Load whiskey data and merge
    whiskey_df = pd.read_csv(os.path.join(DATA_DIR, 'whisky_data.csv'))
    meta_df = pd.merge(meta_df, whiskey_df, on='id', how='left')

    logger.info(f"Loaded and merged {len(meta_df)} metadata entries")
    logger.info(f"Merged columns: {meta_df.columns.tolist()}")
...
def handle_search():
    ...
    try:
        ...
        for score, idx in zip(scores[0], indices[0]):
            ...
            results.append({
                'id': int(result['id']),
                'whiskyName': result['name_x'],
                'score': float(score),
                'spiritType': result.get('spirit_type', ''),
                'abv': str(result.get('abv', '')),
                'proof': str(result.get('proof', '')),
                'size': str(result.get('size', '')),
                'shelfPrice': float(result.get('shelf_price', 0)) if not pd.isna(result.get('shelf_price')) else None,
                'popularityScore': float(result.get('popularity', 0)) if not pd.isna(result.get('popularity')) else None
            })
